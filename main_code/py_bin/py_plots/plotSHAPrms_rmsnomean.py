...
def plotSHAPrms_rmsnomean(data_in={"file":"Urms.txt","folder":"Data","plot_folder":"plots","xlabel":"$y^+$",
                                   "ylabel":"$u'^+$","ylabel2":"$|u_iu_j|^+$","fontsize":18,"figsize_x":10,
                                   "figsize_y":8,"colormap":"viridis","colornum":2,"fig_name":"urms",
                                   "fig_name2":"SHAP_uvabs","dpi":60,"dy":1,"dx":1,"dz":1,
                                   "uvw_folder":"../../P125_21pi_vu/",
                                   "SHAP_uSHAP_vw_file":"P125_21pi_vu.$INDEX$.h5.SHAP_uSHAP_vw",
    ...
    file_nm         = file.replace(".txt","_nomean.txt")
    data_SHAPrms_nm = read_rms(data_in={"folder":folder,"file":file_nm,"dy":dy})
    SHAP_urms_nm    = (data_SHAPrms_nm["SHAP_urms"][:flow_data.yl_s]+
                       np.flip(data_SHAPrms_nm["SHAP_urms"][flow_data.yu_s:]))/2
    SHAP_vrms_nm    = (data_SHAPrms_nm["SHAP_vrms"][:flow_data.yl_s]+
                       np.flip(data_SHAPrms_nm["SHAP_vrms"][flow_data.yu_s:]))/2
    SHAP_wrms_nm    = (data_SHAPrms_nm["SHAP_wrms"][:flow_data.yl_s]+
                       np.flip(data_SHAPrms_nm["SHAP_wrms"][flow_data.yu_s:]))/2
    SHAP_uv_nm      = (abs(data_SHAPrms_nm["SHAP_uv"][:flow_data.yl_s])+
                       abs(np.flip(data_SHAPrms_nm["SHAP_uv"][flow_data.yu_s:])))/2
    SHAP_uw_nm      = (abs(data_SHAPrms_nm["SHAP_uw"][:flow_data.yl_s])+
                       abs(np.flip(data_SHAPrms_nm["SHAP_uw"][flow_data.yu_s:])))/2
    SHAP_vw_nm      = (abs(data_SHAPrms_nm["SHAP_vw"][:flow_data.yl_s])+
                       abs(np.flip(data_SHAPrms_nm["SHAP_vw"][flow_data.yu_s:])))/2
    
    SHAP_mrms_nm    = (data_SHAPrms_nm["SHAP_mrms"][:flow_data.yl_s]+
                       np.flip(data_SHAPrms_nm["SHAP_mrms"][flow_data.yu_s:]))/2
    SHAP_rms_m_nm   = np.sqrt(SHAP_urms_nm**2+SHAP_vrms_nm**2+SHAP_wrms_nm**2)
    ...
    plot_info2 = {"data_x":flow_data.yplus,"data_y":SHAP_urms_nm,"label":"$(\phi_u-\overline{\phi}_u)'$",\
